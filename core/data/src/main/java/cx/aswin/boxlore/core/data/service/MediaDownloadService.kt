...
class MediaDownloadService : DownloadService(
    ...
    override fun getForegroundNotification(
        ...
        val activeTitle = activeDownload?.let { dl ->
            try {
        ...
        val activePercent = if (activeDownload != null && activeDownload.percentDownloaded >= 0f) {
        ...
        val titleText = if (remainingCount > 0) {
            "Syncing Mixtape ($remainingCount left)"
        } else {
            "Syncing Mixtape"
        }

        val contentText = if (activeDownload != null) {
