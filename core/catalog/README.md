...
- `SubscriptionRepository`, `ChapterRepository`, and `TranscriptRepository` expose catalog-adjacent data operations. `ChapterRepository` supports remote JSON, local file paths, and show note description parsing (`hasChaptersInDescription`, `parseChaptersFromDescription`). `TranscriptRepository` supports remote SRT/VTT and local file paths. Offline persistence and file retrieval for downloaded episodes are modularized in `ChapterOfflineStorage` (`saveOfflineChapters`, `getOfflineChapters`, `deleteOfflineChapters`, `hasOfflineChapters`) and `TranscriptOfflineStorage` (`saveOfflineTranscript`, `getOfflineTranscript`, `deleteOfflineTranscript`, `hasOfflineTranscript`). Normal `updateLatestEpisode` calls never replace a newer Room tip with an older item (`LatestEpisodeTipLogic`); same published date does **not** replace or badge. A successful full publisher-feed ingest passes `publisherFeedAuthoritative`, allowing the real feed tip to replace a newer PI cross-promo or same-date different identity. NEW is still set only when `publishedDate` is newer. Subscribe recovers a missing HTTPS `feedUrl` via `GET /podcast` (daily retry also runs in launch sync). When the listener turns **show notifications** on, `SubscriptionRepository` writes RTDB `tracked_podcasts/{podcastIndexId}` (`title`, `imageUrl`, and HTTPS `feedUrl` only when a Room tip exists so the Action can seed `lastRssKey`). Live RTDB rules allow those three keys only. The Check New Episodes Action seeds `lastRssKey` from existing `lastEpisodeId` on first RSS see (notifies only when the PI id differs). Turning notifications **off** removes the `feedUrl` child. Unsubscribe sets a 14-day TTL on the local catalog (no CASCADE wipe), records an `unsubscribedAt` tombstone timestamp, and marks `isDirty = 1` for delta sync; resubscribing resets `unsubscribedAt = 0` and sets `isDirty = 1`. Show settings updates (notifications, custom genre/icon, auto-download, skip intro/outro, playback speed) mark the show `isDirty = 1` for cloud sync. `updateCustomGenre` applies user-defined genre overrides and custom icon keys for subscribed shows; unsubscription clears any custom tag/icon overrides and resets to original catalog genre.
...
- `backup.LibraryBackupManager` imports and exports library data, OPML, listening history, ranking backup payloads, and global prefs including `contentLanguages`, `subscriptionSort` (may be `Manual`), optional `subscriptionManualOrder`, optional `subscriptionFolderSort`, optional `subscriptionIntraFolderSort`, optional `subscriptionFolderManualOrder`, optional `autoOrganizeFolders`, optional `homePinnedPodcastIds`, optional `sameShowQueueOnly`, optional `homeShortcutsInLibrary`, optional `widgetAppearance`, optional `exploreDefaultTab`, optional `subscriptionsDefaultTab`, optional `subscriptionsTabStyle`, optional `navigationStyle`, and optional `fontRoundness`. JSON restore preserves each valid historical `subscribedAt`, custom genre overrides (`customGenre`, `customGenreIcon`), subscription folders (`SubscriptionFolderBackup`), and suppresses new-subscribe ranking feedback; missing/future timestamps fall back to import time. History restore preserves manual/bulk completion provenance while sanitizing legacy Gson-null strings. RSS/OPML restore rethrows coroutine cancellation and treats other failures as skippable. OPML import matches Podcast Index first (feed URL variants, same-title / same-feed search, then a publisher-feed peek for redirected URL + podcast GUID) and creates a true `rss:` row only after those identity lookups settle as not found. Transient PI URL/GUID/search failures defer that feed rather than silently creating an RSS subscription. Every imported PI subscription with an HTTPS publisher `feedUrl` now receives a bounded direct-feed ingest immediately, including old JSON backups whose optional `directFeedOptIns` is absent/null and OPML rows resolved to PI. Incoming **v6** `directFeedOptIns` remains a compatible explicit URL override; PI `/sync` handles only imported PI rows without a usable publisher URL, and true `rss:` rows use `refreshCatalogIfNeeded`. Settings → Add RSS is unchanged (always `rss:`); null/blank Gson `podcastId` / `feedUrl` rows are skipped. Older backups without the Manual-order / pin lists / folder lists still parse; Manual with a missing id list seeds from the current Smart list on first use.
- `backup.LibraryBackupFolderLogic` maps `SubscriptionFolder` to `SubscriptionFolderBackup` and restores folder definitions, memberships, and metadata.
- `SharedAppDependencies` and `SharedAppDependenciesHolder` expose application-scoped instances to workers and services (including `userSyncCoordinator`).
...
- `ports.ListeningHistoryBackupPort`, `ports.SmartDownloadSyncPort`, `ports.QueueSyncPort`, `ports.ActivePlaybackSyncPort`, and `ports.PodcastNotificationSyncPort` keep backup, download, notification topic bindings, and playback seams cleanly decoupled without introducing forbidden `:core:catalog` to `:core:playback` dependency cycles.
- `sync.UserSyncCoordinator` orchestrates Boxlore Realtime Cloud Sync: batch-bounded dirty push with multi-column optimistic concurrency flag clearing, delta pull, cloud account deletion (`deleteCloudSyncData`), account switch local data purge and active playback session teardown via `activePlaybackSyncPort.stopAndClearActiveSession()`, and coordination across `SubscriptionSyncResolver` (LWW subscription and tombstone resolution, RSS ingestion), `HistorySyncResolver` (active playing episode shield, progress LWW, independent like status LWW), and `QueueSyncResolver` (monotonic versioning, 3-way tombstone merge, head-position retention of active episode, and 5-tier metadata hydration). Results are communicated via `sync.SyncResult`, with reactive UI state modelled via `sync.CloudSyncUiStatus`.
