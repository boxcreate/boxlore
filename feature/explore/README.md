...
- Explore search chips: **Find a show** (Meili typeahead + hybrid **Also found**, 300ms debounce) and **By concept** (one CF embed → podcast + episode vectors via `GET /search/semantic`, 1000ms debounce before embed; loader while waiting). Concept idle: same `ExploreIconTitleHeader` pattern as Find a show, plus a structured example-query list. Concept results: **Related shows** rail + **Episodes** header above the hero/bento feed.
