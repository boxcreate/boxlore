...
- `LegacyRssRepair` runs once after onboarding for pre-fix OPML rows. It fetches each subscribed `sourceType=rss` feed and accepts only an exact Podcast Index feed-URL variant or podcast GUID; title/author/artwork are never match inputs. Rows with `linkedPodcastIndexId` are skipped because they represent an intentional RSS-source choice. URL/GUID conflicts, lookup failures, active target subscriptions, or incomplete local catalogs leave the RSS row untouched. Successful rows adopt the PI id while retaining RSS episode ids, journaling DataStore id references for crash-safe completion, and clearing any recommendation cache seeded by the old id. A pass starts only when an eligible row and connectivity are present, publishes `inProgress` for persistent app-shell status UI, and can be requested again after an interrupted/transient pass; the version marker makes completed launches a no-op.
...
- `backup.LibraryBackupManager` imports and exports library data, OPML, listening history, ranking backup payloads, and global prefs including `contentLanguages`, `subscriptionSort` (may be `Manual`), optional `subscriptionManualOrder`, and optional `homePinnedPodcastIds`. RSS/OPML restore rethrows coroutine cancellation and treats other failures as skippable. OPML import matches Podcast Index first (feed URL variants, same-title / same-feed search, then a publisher-feed peek for redirected URL + podcast GUID) and creates a true `rss:` row only after those identity lookups settle as not found. Transient PI URL/GUID/search failures defer that feed rather than silently creating an RSS subscription; PI subscriptions continue using publisher RSS feeds for episode refresh. Settings → Add RSS is unchanged (always `rss:`); the one-time legacy repair above is exact-identity-only. New JSON backups do **not** export an opt-in bit. Incoming **v6** `directFeedOptIns` still mean “rehydrate a local catalog from this HTTPS `feedUrl`” (no supplement stub, `markAsNew` is false). Then PI `/sync` for everyone else and `refreshCatalogIfNeeded` for true `rss:` rows. Null/blank Gson `podcastId` / `feedUrl` rows are skipped. Older JSON without the field stays compatible. Older backups without the Manual-order / pin lists still parse; Manual with a missing id list seeds from the current Smart list on first use.
