...
- Cold-start destination precedence (see `StartDestinationResolver`): incomplete onboarding → offline Downloads (no deep link) → Appearance **Open app to** Subscriptions (no deep link) → Home. When cold start opens Subscriptions, `openedToSubscriptionsOnLaunch` makes Back navigate to Home (not Library hub); Library-tile entry still pops to Library.
