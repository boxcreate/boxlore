...
- On startup, `BoxLoreApplication` hydrates missing DataStore appearance keys from theme fast-cache (Google Backup can restore SharedPreferences without DataStore), then asks `SmartDownloadManager.reconcileScheduleWithPreferences` to keep the Smart Downloads toggle and periodic WorkManager job in sync. It also configures `LearningEventLog` via `BoxcastPrefs.resolveLearnerLogEnabled`: on by default in debug when unset; **always off in release** unless the user has explicitly persisted an opt-in from the debug screen.
- `AppContainer` constructs the shared graph: database, network, RSS, ranking, catalog, playback, queue, downloads, prefs, and analytics dependencies. It also builds `LocalEpisodeCatalogRepository` (listener ids + known tip from Room) and passes it into `PodcastRepository`, `SubscriptionRepository`, `SubscriptionForegroundSync`, and FCM hydration. The same application scope owns `LegacyRssRepair`.
...
- FCM (`BoxLoreFcmService`) owns notification_received / tap extras (`notification_type`, podcast/episode ids for snake+camel keys). Generic push intents propagate those extras so taps are not always `"push"`. For `type=new_episode`, `NewEpisodePushHydration` refreshes the first-class local catalog when wired (guid/enclosure lookup, no newest-in-feed fallback). Otherwise opted-in PI shows still run the extras `refreshFromFeed` path (1000-oldest PI baseline in parallel) so Home chips and Library stay in sync; the payload enclosure/guid still selects the notification episode. If the payload has no usable episode id, the tap opens the podcast page.
...
- `HomeScreenWidgetsInstaller` installs `NowPlayingWidgetDependencies` (via `NowPlayingWidgetPlaybackAdapter`) and `LibraryWidgetDependencies` (via `WidgetLibrarySourceAdapter`) so home-screen widgets share the app-scoped `PlaybackRepository` / library ports without a second graph. Transport calls and cold-session restore hop from the IO application scope to `Dispatchers.Main.immediate` before touching MediaController. Lettering-roundness changes re-render bitmap labels so ROND matches Appearance.
