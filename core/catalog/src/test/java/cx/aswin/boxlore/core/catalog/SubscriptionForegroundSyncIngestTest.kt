...
class SubscriptionForegroundSyncIngestTest {
    ...
    @Test
    fun ingestSubscribedLocalCatalogSkipsRssAndUnsubscribed() =
        runTest {
            var refreshCalls = 0
            val rss =
    ...
    @Test
    fun ingestSubscribedLocalCatalogFallsBackToPiWhenFeedMissing() =
        runTest {
            var piCalls = 0
            val persisted =
                SubscriptionForegroundSyncIngest.ingestSubscribedLocalCatalog(
    ...
    @Test
    fun ingestSubscribedLocalCatalogEmitsRefreshOnlyOnPersist() =
        runTest {
    ...
    @Test
    fun ingestSubscribedLocalCatalogReappliesTtlIfUnsubscribedDuringRefresh() =
        runTest {
            var subscribed = true
            var ttlMarks = 0
            val refreshed = mutableListOf<String>()
    ...
    @Test
    fun ingestSubscribedLocalCatalogSyncsPiWhenRefreshHasNoTip() =
        runTest {
            var piCalls = 0
            val persisted =
                SubscriptionForegroundSyncIngest.ingestSubscribedLocalCatalog(
    ...
    @Test
    fun requestCatalogIngestRunsWhileRefreshIsInFlightAndDuringCooldown() =
        runTest {
            var ingestRuns = 0
            var syncRuns = 0
    ...
    @Test
    fun requestCatalogIngestIgnoresRssIds() =
        runTest {
            var ingestRuns = 0
            val finished = mutableListOf<String>()
            val sync =
    ...
    @Test
    fun requestCatalogIngestEmitsFinishedAfterFailure() =
        runTest {
            val finished = mutableListOf<String>()
            val sync =
                SubscriptionForegroundSync(
    ...
    @Test
    fun sweepExpiredLocalCatalogsNoopsWhenCatalogMissing() =
        runTest {
            SubscriptionForegroundSyncIngest.sweepExpiredLocalCatalogs(catalog = null, nowMs = 9L)
        }

    @Test
    fun sweepExpiredLocalCatalogsInvokesPort() =
        runTest {
    ...
    @Test
    fun sweepExpiredLocalCatalogsIsolatesOrdinaryFailures() =
        runTest {
            val catalog =
    ...
    @Test
    fun sweepExpiredLocalCatalogsRethrowsCancellation() =
        runTest {
            val catalog =
                FakeLocalEpisodeCatalogPort().apply {
