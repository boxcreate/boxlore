...
class PodcastInfoSupplementSupportTest {
    @Test
    fun `refreshMissingEpisodes reloads page zero and reports sourceCount`() = runTest {
    ...
    @Test
    fun `autoOptIn returns null on NoDisconnect`() = runTest {
        val port = FakePort(optedIn = mutableSetOf())
    ...
    @Test
    fun `autoOptIn failure is silent unless announced`() = runTest {
        val port = FakePort(optedIn = mutableSetOf())
        port.optInOutcome =
    ...
    @Test
    fun `unionSearch prefers supplement matches`() = runTest {
    ...
    @Test
    fun `refreshMissingEpisodes surfaces Failure message and still reloads`() = runTest {
        val port = FakePort(optedIn = mutableSetOf("123"))
        port.refreshOutcome =
            EpisodeSupplementOutcome.Failure("Couldn't update episodes from the feed")
        val support =
    ...
    @Test
    fun `refreshMissingEpisodes matches against PI-only baseline not merged extras`() = runTest {
        val port = FakePort(optedIn = mutableSetOf("123"))
        port.refreshOutcome =
            EpisodeSupplementOutcome.Success(
                addedCount = 1,
    ...
    @Test
    fun `shouldRefreshOnOpen is true only for opted-in PI shows`() = runTest {
        val port = FakePort(optedIn = mutableSetOf("123"))
        val support =
            PodcastInfoSupplementSupport(
                episodeSupplementPort = port,
    ...
    @Test
    fun `remount hides extras offer after subscribe`() = runTest {
        val port = FakePort(optedIn = mutableSetOf())
        val support =
            PodcastInfoSupplementSupport(
                episodeSupplementPort = port,
    ...
    @Test
    fun `remount hides extras offer when local catalog is ready`() = runTest {
        val port = FakePort(optedIn = mutableSetOf())
        val support =
            PodcastInfoSupplementSupport(
                episodeSupplementPort = port,
