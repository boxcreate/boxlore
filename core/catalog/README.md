...
- `backup.LibraryBackupManager` imports and exports library data, OPML, listening history, ranking backup payloads, and global prefs including `contentLanguages`. JSON **v6** also exports PI shows opted into **Missing episodes?** (`directFeedOptIns`: podcast id + HTTPS publisher URL). Restore writes the opt-in stub, refreshes those publisher feeds against a 1000-episode PI baseline, promotes the newest tip (`markAsNew`), patches RTDB `feedUrl` when notifications are on, then PI `/sync` for everyone else and `refreshCatalogIfNeeded` for true `rss:` rows. Older JSON without the field stays compatible (no opt-in restore).
