...
interface SmartQueueEngine {
    companion object {
         ...
         */
        val DISCOVERY_REFILL_SOURCES: Set<String> =
            setOf(
                SOURCE_RESUME,
                SOURCE_RESUME_STALE,
                SOURCE_SUBSCRIPTION,
        ...
        /**
         * Explicit show surfaces where same-show binge continuation is expected.
         */
        val SHOW_BINGE_SOURCES: Set<String> =
            setOf(
                "podcast_detail",
                "podcast_info_screen",
                "podcast_info_episodes_list",
                "podcast_info_search_results",
                "home_filtered_latest_episodes",
                "library_downloaded_show_episodes",
                SOURCE_SAME_PODCAST,
            )
...
class DefaultSmartQueueEngine(
    ...
    private suspend fun sameShowContinuation(
        ...
        val allEpisodes =
            runSuspendCatching { sources.getEpisodesAround(podcast.id, currentId, sort) }
        ...
        val pick =
            similar.firstOrNull {
                it.id !in exclude &&
                    ...
                    it.episodeType != "trailer" &&
                    it.audioUrl.isNotBlank()
            } ?: return emptyList()
        exclude.add(pick.id)
    ...
    private suspend fun trendingEpisodes(
        ...
        for (pod in ordered) {
            ...
            feedFetches++
            val next =
                runSuspendCatching { sources.getEpisodes(pod.id) }
                    .getOrDefault(emptyList())
                    .sortedByDescending { it.publishedDate }
                    .firstOrNull { isTrendingCandidate(it, exclude) }
            if (next != null) {
