...
- `FolderEditSheet` and `FolderEditComponents`: Subscription folder creation and edit sheet featuring a streamlined hierarchy, prominent name input with quick-fill suggestions, visual 3×3 grid footprint diagrams for 5 display sizes (`COMPACT 1×1`, `WIDE 2×1`, `FEATURED 2×2`, `LARGE 2×3` max, `SHELF 3×1`), compact optional icon picker row, and auto-sync with genre tags.
