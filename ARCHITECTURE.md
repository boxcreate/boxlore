...
:core:analytics | :core:data | :core:downloads | :core:playback | :core:ranking
...
| `:core:data` | Repositories and RSS (`RssFeedClient`); analytics moved to `:core:analytics` (re-exported via `api`); download stack moved to `:core:downloads`; ranking moved to `:core:ranking` (re-exported via `api`) |
...
Today: `:core:playback`, `:core:domain`, `:core:database`, `:core:network`, `:core:prefs`, `:core:downloads`, `:core:analytics`, and `:core:ranking` are extracted; RSS still lives in `:core:data`.
