...
- Explore search chips: **Find a show** (Meili typeahead + hybrid **Also found**, 300ms debounce) and **By concept** (one CF embed → podcast + episode vectors via `GET /search/semantic`, 1000ms debounce before embed; loader while waiting). Concept idle: short copy + tappable example phrases. Concept results: compact **Related shows** rail above the episode hero/bento feed.
