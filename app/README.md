...
- `AppContainer` constructs the shared graph: database, network, auth (`authRepository`), RSS, ranking, catalog, folders (`FolderRepository`), playback, queue, downloads, prefs, and analytics dependencies. It builds `LocalEpisodeCatalogRepository` with the main Room database and the downloads-owned cache relinker, allowing a repaired full feed to transactionally re-key safely matched legacy listener ids without creating a reverse module dependency. The catalog instance is shared by `PodcastRepository`, `SubscriptionRepository`, `SubscriptionForegroundSync`, and FCM hydration. The same application scope owns `LegacyRssRepair`.
- Authentication & Deep Links: `MainActivity` handles incoming App Link intents for `/boxlore/auth` (`https://aswin.cx/boxlore/auth`) to complete passwordless email link sign-in via `authRepository.signInWithEmailLink`.
