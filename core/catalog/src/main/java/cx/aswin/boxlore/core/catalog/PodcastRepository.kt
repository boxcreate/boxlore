...
class PodcastRepository(
    ...
    suspend fun searchPodcastsWithCorrection(query: String): SearchResult =
        withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun searchEpisodesSemantic(
        query: String,
        country: String,
    ): List<Episode> =
        withContext(Dispatchers.IO) {
    ...
    private suspend fun getRssPodcastDetails(feedId: String): Podcast? =
        try {
    ...
    private fun mapPodcastResponseFeed(feed: cx.aswin.boxlore.core.network.model.PodcastFeed): Podcast =
        Podcast(
            id = feed.id.toString(),
            title = feed.title,
            artist = feed.author ?: "Unknown",
    ...
    private suspend fun getNetworkPodcastDetails(feedId: String): Podcast? =
        try {
    ...
    suspend fun getContentCatalog(forceRefresh: Boolean = false): cx.aswin.boxlore.core.catalog.content.ContentCatalogSnapshot? =
        withContext(Dispatchers.IO) {
            val now = System.currentTimeMillis()
    ...
    suspend fun getPodcastType(feedId: String): String =
        withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun getPodcastMeta(feedId: String): cx.aswin.boxlore.core.network.model.PodcastMetaResponse? =
        withContext(Dispatchers.IO) {
            try {
                val response =
                    when {
    ...
    suspend fun getBriefingMetadata(region: String): Briefing? =
        withContext(Dispatchers.IO) {
    ...
    suspend fun getHomeBootstrapDataFast(country: String): HomeBootstrapData =
        withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun getCuratedCuriosity(
        page: Int = 1,
        bypassCache: Boolean = false,
    ): CuratedCuriosityResponseDto? =
        withContext(Dispatchers.IO) {
            try {
