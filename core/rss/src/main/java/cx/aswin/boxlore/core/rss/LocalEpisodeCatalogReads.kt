...
internal class LocalEpisodeCatalogReads(
    ...
    override suspend fun getPage(podcastId: String, limit: Int, offset: Int, sort: String, meta: PodcastMeta,): List<Episode> = withContext(Dispatchers.IO) {
        val rows =
            if (sort == "oldest") {
    ...
    override suspend fun getEpisode(episodeId: String, meta: PodcastMeta,): Episode? = withContext(Dispatchers.IO) {
        dao.getEpisode(episodeId)?.toCatalogEpisode(meta)
    }

    override suspend fun findByCatalogKey(podcastId: String, guid: String?, enclosureUrl: String?, meta: PodcastMeta,): Episode? = withContext(Dispatchers.IO) {
    ...
    override suspend fun search(podcastId: String, query: String, meta: PodcastMeta,): List<Episode> = withContext(Dispatchers.IO) {
        val trimmed = query.trim()
        if (trimmed.isEmpty()) return@withContext emptyList()
        dao.search(podcastId, trimmed.escapeForSqlLike()).map { it.toCatalogEpisode(meta) }
    }

    override suspend fun newest(podcastId: String, meta: PodcastMeta,): Episode? = withContext(Dispatchers.IO) {
    ...
    override suspend fun isPublisherFeedUnchanged(podcastId: String, feedUrl: String,): Boolean = withContext(Dispatchers.IO) {
    ...
    override suspend fun refresh(request: RefreshRequest): RefreshOutcome = RefreshOutcome.Failure(LocalEpisodeCatalogRepository.FEED_LOAD_FAILED_MESSAGE)

    override suspend fun markFeedUrlLookup(podcastId: String, atMillis: Long,) = withContext(Dispatchers.IO) {
    ...
    override suspend fun lastFeedUrlLookupAt(podcastId: String): Long = withContext(Dispatchers.IO) {
        dao.getFeed(podcastId)?.feedUrlLookupAt ?: 0L
    }

    override suspend fun setUnsubscribedTtl(podcastId: String, ttlExpiresAt: Long?,) = withContext(Dispatchers.IO) {
...
internal fun LocalEpisodeEntity.toCatalogEpisode(meta: PodcastMeta): Episode = toEpisode(
