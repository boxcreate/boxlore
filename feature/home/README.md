...
- `settings.SettingsScreen`, `SettingsViewModel`, and `SettingsViewModelAssembler` for Settings. Hub category cards center title and description with the leading icon (descriptions are not forced to two lines). Shared settings rows keep icons vertically centered with their labels; playback duration sliders keep the icon on the heading row so the slider can use the full width. Section labels line up with that same inner padding. **Account** settings page (`AccountSettingsPage`) displays live cloud sync status (`CloudSyncUiStatus`: idle, syncing, relative sync time via `formatRelativeSyncTime`, or error) with an interactive "Sync Now" action with rotation animation and haptics, and supports Google One Tap (Credential Manager), email link magic auth (`https://aswin.cx/boxlore/auth`), and email/password sign-in and account management. Sign-up includes confirm password verification, password inputs preserve whitespace for passphrases, client-side email pre-validation prevents bad network calls, and auth state survives configuration changes via `rememberAccountAuthState`. Account deletion prompts for sign-out and re-authentication when recent authentication is required. Library **Discovery** settings use a Material 3 region sheet (11 storefronts) plus grouped language chips in one block (`content_languages`; English locked, max 4; reset on region change). Full library JSON backup (v6) includes PI shows opted into **Missing episodes?**; restore re-opts those feeds in and refreshes catalog tips. Appearance includes Theme (System / Light / Dark connected toggle), Background, **Colors** (wallpaper colors or a fixed accent; custom picker can keep a Material 3 seed or pin the exact pick — not recommended / not tested for UI readability), **Lettering** (Crisp / Soft / Round connected toggle, same chip language as content region, plus an expandable sample preview), Navigation (Floating / Classic), **Open app to** (Home / Subscriptions / Downloads cold-start landing; Back from launch-Subscriptions or launch-Downloads goes to Home), **Default tabs** (Explore For You or Top; Subscriptions Shows or New episodes — used when the route does not already pick a tab), **Subscriptions tabs** (Top inside header or Floating pill at the bottom), **Cleaner Home** (off by default; moves Settings and Feedback from the Home top bar to Library), and **Widgets** (App theme by default, or System for launcher light/dark and wallpaper accents). Playback includes **Restart forgotten episodes** (default on; soft-expires mid-episode seek for queue/mixtape after 7 days — see `:core:playback` `PlaybackSkipPolicy`) and **Smart queue** (default on; turning it off makes Smart Queue append only newer/next episodes of the current show). Mixtape cards hide progress chrome for soft-expired picks; Smart Queue labels those fills “Starting over”.
