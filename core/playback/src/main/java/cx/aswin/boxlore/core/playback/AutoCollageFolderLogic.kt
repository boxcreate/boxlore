...
object AutoCollageFolderLogic {
    data class ArtPair(val contentKey: String, val imageUrl: String,)

    fun takeAligned(pairs: List<ArtPair>, limit: Int = 4,): List<ArtPair> = pairs.filter { it.contentKey.isNotBlank() && it.imageUrl.isNotBlank() }.take(limit)
    ...
    fun pairOrNull(contentKey: String?, imageUrl: String?,): ArtPair? {
