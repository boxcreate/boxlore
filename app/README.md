...
- `configureNowPlayingWidget` installs `NowPlayingWidgetDependencies` (via `NowPlayingWidgetPlaybackAdapter`) so all three home-screen playback widget families share the app-scoped `PlaybackRepository` without constructing a second graph. Transport calls hop to `Dispatchers.Main` (MediaController thread rule). Lettering-roundness changes re-render bitmap labels so ROND matches Appearance settings across launcher hosts.
- `configureLibraryWidgets` installs `LibraryWidgetDependencies` (via `WidgetLibrarySourceAdapter`) so Subscriptions / New Episodes widgets mirror Library → Subscriptions sorting and filters without importing `:feature:library`.
